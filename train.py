...
from fukaya_seidel import fukaya_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = fukaya_score(
            ...
            macro_window,
            threshold=config.FLOER_THRESHOLD,
            a_order=config.A_INFITY_ORDER
        )
    ...
    top_etfs = [{"ticker": t, "fukaya_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "floer_threshold": config.FLOER_THRESHOLD,
        "a_infinity_order": config.A_INFITY_ORDER,
    ...
    out_file = f"output/fukaya_{timestamp}.json"
