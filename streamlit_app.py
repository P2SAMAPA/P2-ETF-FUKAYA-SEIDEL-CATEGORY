...
st.set_page_config(page_title="Fukaya-Seidel Category", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Fukaya-Seidel Category</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">A∞ category | Lagrangians in symplectic manifolds | Floer homology | Portfolio mutations</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Fukaya-Seidel")
...
st.sidebar.markdown(f"**Floer threshold:** {config.FLOER_THRESHOLD} | **A∞ order:** {config.A_INFITY_ORDER}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'fukaya_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Floer rank: {etf['fukaya_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Floer Rank"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Floer Rank", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Fukaya-Seidel Floer Rank (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Fukaya-Seidel category** represents assets as Lagrangians in a symplectic manifold.
        - **Floer homology** detects stable pairings (intersections) between Lagrangians.
        - **A∞ operations** encode higher-order associative relationships between assets.
        - **Mutations** correspond to portfolio rebalancing.
        - The score is the **Floer homology rank** (number of stable intersections).
        - Higher score → more stable market structure → potential alpha.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the Fukaya-Seidel scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Fukaya-Seidel Category | A∞ category for ETF relationships")
